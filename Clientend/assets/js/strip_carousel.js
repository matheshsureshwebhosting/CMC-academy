...
$(document).ready(async function () {
  ...
  await db.collection("gallery").get().then((snap) => {
    var data = []
    snap.forEach((doc) => {
      if (doc.data() !== undefined) {
        data.push(doc.data())
        document.getElementById("galleryframes").innerHTML += ` <a href="https://youtu.be/YfDFKgGNP6s" target="_blank" >
      <div class="frame" style="background:url('${doc.data().gallery}');background-size: cover;" ></div>
    </a>`
      }
    })
    if (data.length == 0) {
      for (var i = 0; i < 7; i++) {
        document.getElementById("galleryframes").innerHTML += ` <a  href="https://youtu.be/YfDFKgGNP6s" target="_blank" >
        <div class="frame" style="background:url('../img/back7.jpg');background-size: cover;" ></div>
      </a>`
      }
    }
  })
  ...
  */
  ...
  function set_frame_size() {
    ...
    if (browser_width < 500) {
      calc_frame_width = browser_width - ((browser_width / 100) * 20) - 20;
      calc_frame_height = (browser_width / 100) * 40;
    }
    else if (browser_width > 500 && browser_width < 768) {
    ...
    // set Carousel height
    $(".strip-carousel").css({
      "height": calc_frame_height + 15,
      "min-height": calc_frame_height + 15
    });
